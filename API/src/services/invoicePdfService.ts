...
export async function generateInvoicePDF(invoiceData: InvoiceData): Promise<string> {
  ...
  return new Promise((resolve, reject) => {
    ...
    // Logo
    if (invoiceData.logoPath && fs.existsSync(invoiceData.logoPath)) {
      try {
    ...
    invoiceData.lineItems.forEach((item, index) => {
      ...
      doc.fillColor(darkColor).fontSize(9).font('Helvetica')
        ...
        .text(formatCurrency(item.rate), colX[3], y + 6, { width: colWidths[3] })
        .text(formatCurrency(item.amount), colX[4], y + 6, { width: colWidths[4] });
    ...
    const addTotalRow = (label: string, amount: number, bold = false, color = darkColor) => {
      ...
      doc.fillColor(color).fontSize(9).font(bold ? 'Helvetica-Bold' : 'Helvetica')
        .text(formatCurrency(amount), totalsX + 100, y, { width: 85, align: 'right' });
      y += 18;
    };
    ...
    if (invoiceData.cgstAmount) addTotalRow(`CGST (${invoiceData.cgstRate}%):`, invoiceData.cgstAmount);
    if (invoiceData.sgstAmount) addTotalRow(`SGST (${invoiceData.sgstRate}%):`, invoiceData.sgstAmount);
    if (invoiceData.igstAmount) addTotalRow(`IGST (${invoiceData.igstRate}%):`, invoiceData.igstAmount);
    ...
    doc.fillColor('white').fontSize(12).font('Helvetica-Bold')
      ...
      .text(formatCurrency(invoiceData.totalAmount), totalsX + 100, y + 8, { width: 85, align: 'right' });
