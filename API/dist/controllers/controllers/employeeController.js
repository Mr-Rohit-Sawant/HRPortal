...
const createEmployee = async (req, res) => {
    ...
    const profilePhoto = files?.profilePhoto?.[0] ? `uploads/profiles/${files.profilePhoto[0].filename}` : null;
    const cvFile = files?.cvFile?.[0] ? `uploads/employee-cvs/${files.cvFile[0].filename}` : null;
    const cvOriginalName = files?.cvFile?.[0]?.originalname ?? null;
...
const updateEmployee = async (req, res) => {
    ...
    if (files?.profilePhoto?.[0]) {
        updates.profilePhoto = `uploads/profiles/${files.profilePhoto[0].filename}`;
    ...
    if (files?.cvFile?.[0]) {
        updates.cvFile = `uploads/employee-cvs/${files.cvFile[0].filename}`;
        updates.cvOriginalName = files.cvFile[0].originalname;
    }
