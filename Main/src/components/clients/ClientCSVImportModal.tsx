...
const STANDARD_FIELDS = [
  ...
  { key: 'contractStartDate', label: 'Contract Start Date (YYYY-MM-DD)' },
  { key: 'contractEndDate',   label: 'Contract End Date (YYYY-MM-DD)' },
...
const SAMPLE_VALUES: Record<string, string> = {
  companyName: 'Acme Corp', contactPerson: 'John Doe', email: 'contact@acme.com',
  phone: '+919876543210', alternatePhone: '', industry: 'IT',
  website: 'https://acme.com', address: '123 Business Park', city: 'Mumbai',
  state: 'Maharashtra', country: 'India', pincode: '400001',
  gstNumber: '27AAPFU0939F1ZV', panNumber: 'AAPFU0939F',
  contractStartDate: '2024-01-01', contractEndDate: '2025-01-01', notes: '',
};

interface ImportResult { row: number; name: string; status: 'success' | 'error'; message?: string; }

export default function ClientCSVImportModal({ onClose }: { onClose: () => void }) {
  ...
  const { data: customColsData } = useQuery({
    queryKey: ['columns', 'clients'],
    queryFn: async () => { const res = await settingsService.getColumns('clients'); return (res.data.data || []) as ColumnDefinition[]; },
  });
  ...
  const downloadTemplate = () => {
    ...
    const a = document.createElement('a'); a.href = url; a.download = 'client_import_template.csv';
      ...
      complete: async ({ data: rows }) => {
        ...
        for (let i = 0; i < rows.length; i++) {
          ...
          Object.entries(row).forEach(([header, val]) => {
            ...
            if (fieldKey) data[fieldKey] = val;
            else if (customCol) customFieldValues[customCol.name] = val;
          });

          const bizId = isSuperAdmin ? selectedBusinessId : (user?.businessId || '');
          if (bizId) data.businessId = bizId;

          const name = data.companyName || `Row ${i + 1}`;

          try {
            const createRes = await clientService.createClient(data as any);
            ...
            if (createdId && Object.keys(customFieldValues).length > 0) {
              await Promise.allSettled(
                Object.entries(customFieldValues).map(([fieldName, value]) =>
                  api.patch(`/clients/${createdId}/custom-fields`, { fieldName, value })
                )
              );
            }
        ...
        if (ok > 0) toast.success(`${ok} client${ok > 1 ? 's' : ''} imported`);
      },
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4">
      <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-2xl border border-slate-200 dark:border-slate-700 w-full max-w-lg">
        <div className="flex items-start justify-between px-5 py-4 border-b border-slate-100 dark:border-slate-700">
          <div>
            <h2 className="text-base font-semibold text-slate-900 dark:text-white">Import Clients via CSV</h2>
            <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">Download the template, fill client data, then upload</p>
          </div>
          ...
          {!canProceed ? (
            ...
            <div className="flex items-center gap-3 p-3.5 bg-primary-50 dark:bg-primary-900/20 rounded-xl border border-primary-100 dark:border-primary-800/50">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-slate-500 mt-0.5">{totalCols} columns{customCols.length > 0 ? ` (${customCols.length} custom)` : ''}</p>
              </div>
                ...
                {importing && (
                  <div className="space-y-2.5">
                    <div className="flex items-center justify-between text-xs text-slate-600 dark:text-slate-400">
                      <span className="flex items-center gap-1.5"><Loader2 size={12} className="animate-spin text-primary-600" />Importing clients...</span>
