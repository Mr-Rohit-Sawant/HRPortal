...
const SAMPLE_VALUES: Record<string, string> = {
  ...
  username: '', phone: '+919876543210', department: 'Engineering',
  designation: 'Software Engineer', roleName: 'recruiter',
  joiningDate: '2024-01-15', salary: '600000',
  address: '123 Main St', city: 'Mumbai', state: 'Maharashtra', country: 'India',
};

interface ImportResult { row: number; name: string; status: 'success' | 'error'; message?: string; }

export default function EmployeeCSVImportModal({ onClose }: { onClose: () => void }) {
  ...
  const { data: customColsData } = useQuery({
    queryKey: ['columns', 'employees'],
    queryFn: async () => { const res = await settingsService.getColumns('employees'); return (res.data.data || []) as ColumnDefinition[]; },
  });
  const customCols = useMemo(() => (customColsData || []).filter(c => c.isVisible), [customColsData]);

  const roleMap = useMemo(() => {
    const map: Record<string, string> = {};
    (rolesData || []).forEach((r: any) => { map[r.name.toLowerCase()] = r.id; map[r.id] = r.id; });
    return map;
  }, [rolesData]);

  const downloadTemplate = () => {
    const headers = [...STANDARD_FIELDS.map(f => f.label), ...customCols.map(c => c.label)];
    const sample = [...STANDARD_FIELDS.map(f => SAMPLE_VALUES[f.key] ?? ''), ...customCols.map(() => '')];
      ...
      complete: async ({ data: rows }) => {
        ...
        for (let i = 0; i < rows.length; i++) {
          ...
          Object.entries(row).forEach(([header, val]) => {
            ...
            if (fieldKey) {
              if (fieldKey === 'roleName') {
                const roleId = roleMap[val.toLowerCase()] || roleMap[val];
          ...
          if (bizId) fd.append('businessId', bizId);
          ...
          try {
            const createRes = await employeeService.createEmployee(fd);
            ...
            if (createdId && Object.keys(customFieldValues).length > 0) {
              await Promise.allSettled(
                Object.entries(customFieldValues).map(([fieldName, value]) =>
                  api.patch(`/employees/${createdId}/custom-fields`, { fieldName, value })
                )
              );
            }
        ...
        if (ok > 0) toast.success(`${ok} employee${ok > 1 ? 's' : ''} imported`);
      },
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4">
      <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-2xl border border-slate-200 dark:border-slate-700 w-full max-w-lg">
        <div className="flex items-start justify-between px-5 py-4 border-b border-slate-100 dark:border-slate-700">
          <div>
            <h2 className="text-base font-semibold text-slate-900 dark:text-white">Import Employees via CSV</h2>
            <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">Download the template, fill employee data, then upload</p>
          </div>
          <button onClick={onClose} disabled={importing} className="p-1.5 hover:bg-slate-100 dark:hover:bg-slate-700 rounded-lg text-slate-400">
            <X size={16} />
          </button>
        </div>
          ...
          {!canProceed ? (
            ...
            <div className="flex items-center gap-3 p-3.5 bg-primary-50 dark:bg-primary-900/20 rounded-xl border border-primary-100 dark:border-primary-800/50">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-slate-500 mt-0.5">{totalCols} columns{customCols.length > 0 ? ` (${customCols.length} custom)` : ''} — roleName maps to role automatically</p>
              </div>
            ...
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1 min-w-0">
                ...
                {!importing && !isDone && (
                  <div {...getRootProps()} className={cn('border-2 border-dashed rounded-xl p-6 text-center cursor-pointer transition-colors', isDragActive ? 'border-primary-400 bg-primary-50 dark:bg-primary-900/20' : 'border-slate-200 dark:border-slate-600 hover:border-primary-300 hover:bg-slate-50 dark:hover:bg-slate-700/30')}>
                    <input {...getInputProps()} />
                ...
                {importing && (
                  <div className="space-y-2.5">
                    <div className="flex items-center justify-between text-xs text-slate-600 dark:text-slate-400">
                      <span className="flex items-center gap-1.5"><Loader2 size={12} className="animate-spin text-primary-600" />Importing employees...</span>
